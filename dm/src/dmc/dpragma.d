...
static if (0)
{
int phstring_t_cmp(phstring_t s2, int function(void *,void *) func)
{   int result = 0;
    for (int i = 0; 1; ++i)
    {
        if (i == length())
...
void phstring_t_push(const(char)* s)
{
    if (dim == 0)
    {
...
void phstring_t_free(list_free_fp freeptr)
{
    if (dim)
    {
        if (freeptr)
...
int phstring_t_find(const(char)* s)
{
