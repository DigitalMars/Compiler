...
#ifdef DEBUG
	unsigned char *puchOpcode, unsigned short *pusIdx,
#endif
	code *pc,
	...
	unsigned *pusNumops )
{
	...
	else
	{
	    ...
	    {
		popnd2->usFlags = usFlags2 =
		    asm_determine_operand_flags( popnd2 );
		if (!popnd3)
		    usNumops = 2;
		else
		{
		    popnd3->usFlags = usFlags3 =
			asm_determine_operand_flags( popnd3 );
		    usNumops = 3;
		}
	    }
	}
...
unsigned short compute_hashkey(char *s)
{
...
//	printf("Compute Hashkey/%s/",s);
	for (i = j = 1; *s != '\0'; ++s, ++j)
		i = (i << 1) ^ *s;
//	printf( "i=%d, j=%d, i*j=%d (unsigned short) (i*j)=%d\n",
//		i, j, i*j, (unsigned short) (i*j) );
	return (i * j);
}
