...
{ type *at;
  ...
  if (CPP)
  {
	...
	modifiers = e1->ET->Tty & (mTYconst | mTYvolatile T80x86(| mTYLINK));
	thisptr = 1;
	ethis = exp2_addr(e1);
    }
    else
...
elem *builtinFunc(elem *ec)
{
...
#endif
	{
...
#else
	    i = binary(s->Sident T80x86( + 8),inlinefunc,arraysize(inlinefunc));
#endif
	    if (i != -1)
	    {   int op = opcode[i];
		...
		if (OTunary(op))		/* if unary operator		*/
		{
		    ...
		    else if (tybasic(t->Tty) != T80x86(ty1[i]) T68000(TYldouble))
			goto ret;
#endif
...
doit:
  if (ety == TYstruct || ty == TYstruct)
  {
    ...
    ety = e->ET->Tty & (mTYconst | mTYvolatile T80x86(| mTYLINK));
    e = cast(e,t);
...
{   tym_t ty1,ty2;
	...
	{
	    u = type_copy(t->Tnext);
	    u->Tty |= (t1->Tnext->Tty | t2->Tnext->Tty) & (mTYconst | mTYvolatile T80x86(| mTYLINK));
