 ...
 * Source:      https://github.com/DigitalMars/Compiler/blob/master/dm/src/dmc/speller.c
 */
        ...
        const char *charset, int flag)
{
    ...
    for (int i = 0; i < seedlen; i++)
    {
        ...
        void *p;
        if (flag)
            p = spellerX(buf, seedlen - 1, fp, fparg, charset, flag - 1);
        else
            p = (*fp)(fparg, buf);
        if (p)
            return p;
    ...
    for (int i = 0; i + 1 < seedlen; i++)
    {
        ...
        void *p;
        if (flag)
            p = spellerX(buf, seedlen, fp, fparg, charset, flag - 1);
        else
            p = (*fp)(fparg, buf);
        if (p)
            return p;
