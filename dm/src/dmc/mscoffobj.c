...
void MsCoffObj_staticctor(Symbol *s,int dtor,int none)
{
    ...
    //dbg_printf("MsCoffObj_staticctor(%s) offset %x\n",s->Sident,s->Soffset);
    //symbol_print(s);
    s->Sseg = seg =
        MsCoffObj_getsegment(".ctors", null, SHT_PROGDEF, SHF_ALLOC|SHF_WRITE, 4);
    buf = SegData[seg]->SDbuf;
    if (I64)
        buf->write64(s->Soffset);
    else
        buf->write32(s->Soffset);
    MsCoffObj_addrel(seg, SegData[seg]->SDoffset, s, RELaddr);
...
void MsCoffObj_staticdtor(Symbol *s)
{
    ...
    //dbg_printf("MsCoffObj_staticdtor(%s) offset %x\n",s->Sident,s->Soffset);
    //symbol_print(s);
    seg = MsCoffObj_getsegment(".dtors", null, SHT_PROGDEF, SHF_ALLOC|SHF_WRITE, 4);
    buf = SegData[seg]->SDbuf;
    if (I64)
        buf->write64(s->Soffset);
    else
...
void MsCoffObj_setModuleCtorDtor(Symbol *sfunc, bool isCtor)
{
    //dbg_printf("MsCoffObj_setModuleCtorDtor(%s) \n",s->Sident);
}
