...
/* The easiest way to test this non-trivial revision is to compare it with the
    ...
    void asgcpelems(elem *n)
    {
        while (1)
        {
            ...
            else if (OTbinary(op))
            {
                 ...
                 * same variable.
                 * Exclude SFLlivexit variables because may want to cache them
                 * in a register variable, and cp will prevent that.
                ...
                if ((op == OPeq || op == OPstreq) &&
                    ...
                    !((e1.EV.Vsym.Sflags | e2.EV.Vsym.Sflags) & SFLlivexit) &&
                    (!config.fpxmmregs ||
