...
private param_t *template_parameter_list()
{
    ...
    if (tok.TKval != TKlt)
    {
        if (tok.TKval == TKlg)          // if <> token
        {
            ...
            cpperr(EM_lt_following,"template".ptr);         // '<' expected
    }
    if (stoken() == TKgt)
    {   stoken();
        return null;
    ...
    while (1)
    {
        ...
        {   // type-parameter
            ...
            {
                p.Pident = cast(char *) MEM_PH_STRDUP(tok.TKid);   // copy in identifier
            ...
            {   type *t;
                ...
                if (p.Pident)
                {   s = template_createsym(p.Pident, null, cast(Symbol **)&scope_find(SCTtemparg).root);
        ...
        else if (tok.TKval == TKtemplate)
        {
            ...
            if (tok.TKval == TKeq)
            {
                ...
                if (s2)
                {
                    if (s2.Sclass != SCtemplate)
                    {
                        cpperr(EM_class_template_expected, &s2.Sident[0]); // only class templates allowed
                        s2 = null;
                    }
                }
            }
        ...
        L1:
            ...
            pstate.STintemplate++;
            type_specifier(&pt);
            p.Ptype = declar(pt,&vident[0],0);
            fixdeclar(p.Ptype);
            type_free(pt);
            pstate.STintemplate--;
            if (vident[0])      // if there was an identifier
                p.Pident = cast(char *) MEM_PH_STRDUP(&vident[0]);
static if (0)
{
            // No longer an error
            if (!vident[0])
            ...
            if (tyfloating(tym) || tym == TYstruct || tym == TYvoid)
                cpperr(EM_param_no_float,&vident[0]);       // no floating types
            ...
            if (p.Pident)
            {   s = template_createsym(p.Pident, p.Ptype, cast(Symbol **)&scope_find(SCTtemparg).root);
                s.Ssequence = sequence;
            }
        }
        ...
        uint access_specifier)
{
    ...
    int structflags = 0;
    enum_TK tk = cast(enum_TK)tok.TKval;
...
pstate.STmaxsequence = pstatesave.STmaxsequence;
        strcpy(&vident[0],tok.TKid);                // make our local copy
        stoken();
        ...
        {   // Handle things like:
            ...
            token_unget();
            token_setident(&vident[0]);
            token_t* tkx = token_funcbody(false);         // gather the A<T>::B

            /* template<class T> friend class A<T*>; is a partial
             * specialization and is illegal
             */
            token_t *to = tkx.TKnext;
            if (to.TKval == TKlg)
            { }
            else
            {   int brack = 0;
            ...
            if (to && to.TKnext && to.TKnext.TKval == TKsemi)
                cpperr(EM_friend_partial, &s.Sident[0]);

            tmnf = cast(TMNF *) MEM_PH_MALLOC(TMNF.sizeof);
            tmnf.tdecl = tkx;
        ...
        if (tok.TKval != TKsemi)
        {
            ...
            cpperr(EM_semi_template, &vident[0]);                   // ';' expected
            goto Lpanic;
        }
        if (s)
        {
            if (!paramlstmatch(temp_arglist,s.Stemplate.TMptpl))
                cpperr(EM_templ_param_lists,&s.Sident[0]); // parameter lists must match
        }
        else
        {
            // Define forward reference to class template
            s = template_define(null, null, tk, structflags, &vident[0], temp_arglist, null);
            temp_arglist = null;
        }
//printf("Adding '%s' to '%s'.TMfriends\n", stag.Sident, s.Sident);
    ...
    {
        s = n2_searchmember(cast(Classsym *)stag, tok.TKid);
    }
    else
    {
...
static if (0)
{
    ...
    {   param_t *p;
        ...
        if (!s)
        {
            ...
            cpperr(EM_primary_template_first, &vident[0]);
            goto Lpanic;
        }
        ...
        for (p = ptal; p; p = p.Pnext)
        {
            ...
            {
                if (e.Eoper == OPvar)
                {   char *id = &e.EV.Vsym.Sident[0];
                    param_t *px = temp_arglist.search(id);
                    if (px)
                    {
                        Symbol *sv = symbol_calloc(id);
                        sv.Ssequence = e.EV.Vsym.Ssequence;
                        sv.Sclass = e.EV.Vsym.Sclass;
                        sv.Stype = e.EV.Vsym.Stype;
                        sv.Stype.Tcount++;
                        e.EV.Vsym = sv;
                    }
                }
...
static if (1) // Gives errors anyway
{
...
//type_print(e.ET);
                ...
                //if (!typematch(e.ET, pp.Ptype, 0))
                    //cpperr(EM_dependent_specialization, &s.Sident[0]);
}
            }
            pp = pp.Pnext;
        }
...
static if (0)
{
        ...
        if (match)
        {
            cpperr(EM_identical_args, &s.Sident[0]);
        }
...
static if (0)
{
        ...
        if (tok.TKval == TKcolcol && !temp_arglist)
        {
            ...
            char[2*IDMAX + 1] videntx = void;
            strcpy(&videntx[0],tok.TKid);            // make our local copy
            ...
            {   // template<> template<> class A<int>::B<double>;
                ...
                tla.term();
                TMNE *tmne = cast(TMNE *) MEM_PH_CALLOC(TMNE.sizeof);

                tmne.tk = tk;
                tmne.name = cast(char *) MEM_PH_STRDUP(&videntx[0]);    // copy in identifier
                tmne.ptal = ptal;
...
static if (0)
{
                if (tok.TKval == TKsemi)
                {
                    ...
                    cpperr(EM_semi_template, &videntx[0]);   // ';' expected
}
            ...
            TME *tme = cast(TME *) MEM_PH_CALLOC(TME.sizeof);
            Symbol *se;
            ...
            se = symbol_calloc(&videntx[0]);
            se.Sclass = SCtemplate;

            se.Stemplate = cast(template_t *) MEM_PH_CALLOC(template_t.sizeof);
    ...
    {   // Template declaration
        ...
        {
            // Define forward reference to class template
            s = template_define(stag, sprimary, tk, structflags, &vident[0], temp_arglist, ptal);
...
static if (0)
{
            ...
            {
                synerr(EM_multiple_def,&s.Sident[0]); // already defined
        ...
        switch (to.TKval)
        {
            case TKident:
                ...
                if (!brack && !foundident)
                {
                    ...
                    {
                        s = n2_searchmember(cast(Classsym *)s, to.TKid);
                    }
                    else if (s && s.Sclass == SCnamespace)
                    {
                        s = nspace_searchmember(to.TKid, cast(Nspacesym *)s);
                    }
                    else
                                ...
                                while (1)
                                {
                                    ...
                                    if (tox.TKnext.TKval == TKident)
                                    {
                                        if (tox.TKnext.TKnext.TKval == TKcolcol)
                                            continue;
                                        strcpy(&vident[0], tox.TKnext.TKid);
                                        goto L1;
                                    }
                                    else if (tox.TKnext.TKval == TKoperator)
                                    {
                                        strcpy(&vident[0], "operator");
                                        goto L1;
                                    }
                                    break;
                                }
            ...
            L1:
                if (!s)
                {
                    cpperr(EM_not_class_templ,&vident[0]);          // '%s' is not a class template
                    goto syntax_error;
                }
                            ...
                            {   char *id = p1.Pident;
                                ...
                                if (p1.Ptype)
                                {
                                    if (!p2.Pelem ||
                                        ...
                                        strcmp(id, &p2.Pelem.EV.Vsym.Sident[0]))
                                        goto Lerr1;
                                }
                                else if (p1.Pptpl)
                                {
                                    if (!p2.Psym ||
                                        strcmp(id, &p2.Psym.Sident[0]))
                                        goto Lerr1;
                                }
                                else if (!p2.Ptype ||
...
static if (0)
{
                     ...
                     */
                    TMF *tmf = cast(TMF *) MEM_PH_MALLOC(TMF.sizeof);
                    tmf.stag = stag;
                    tmf.tbody = tbody;
                    tmf.to = to;
...
static if (0)
{
                    printf("Adding TMF %p to template '%s'", tmf, &s.Sident[0]);
                    if (tmf.name)
                        printf(" name='%s'", tmf.name);
                    if (member_template & 2)
                        printf(" as friend of stag '%s'", &stag.Sident[0]);
                    printf("\n");
}
                    if (ptal)
                    {
            ...
            case TKlt:
                ...
                {   int brackx = 1;
                    ...
                    {   to = to.TKnext;
                        switch (to.TKval)
                        {   case TKlt:  brackx++;        continue;
                            case TKgt:  if (parenx)      continue;
                                        if (--brackx)    continue;
                                        break;
static if (0)
                            case TKshr: if ((brackx -= 2) > 0) continue;

                            case TKlpar: parenx++;       continue;
                            case TKrpar: parenx--;       continue;
                            default:    continue;
                        }
                        break;
                    }
  ...
  bool more()
  {
    ...
    {   enum_TK tk;
        ...
        if (tok.TKval == TKident && template_classname(tok.TKid, stag))
        {
            tk = stoken();
            token_unget();
            token_setident(stag.Sstruct.Stempsym ? &stag.Sstruct.Stempsym.Sident[0] : &stag.Sident[0]);
            if (tk == TKlpar)
            {
                //printf("constructor\n");
                constructor = 1;
                ...
                t = declar_fix(typ_spec, &vident[0]);
                type_free(t.Tnext);
    ...
    if (!constructor)
    {
        ...
        {
            if (class_m & mskl(SCstatic))
                synerr(EM_storage_class, "static".ptr);
            class_m |= mskl(SCstatic);
        }
        pstate.STintemplate++;
        t = declar_fix(typ_spec,&vident[0]);
    ...
    if (!temp_arglist && gdeclar.oper == OPMAX && gdeclar.class_sym)
    {
        ...
        if (!sftemp)
        {
            synerr(EM_undefined, &vident[0]);
            type_free(t);
            return true;
        }
        s = symbol_name(&vident[0], SCftexpspec, t);
    ...
    else if (!temp_arglist /*&& gdeclar.ptal*/)
    {
        ...
        else
        {
            ...
            {
                sftemp = n2_searchmember(gdeclar.class_sym, &vident[0]);
            }
            else if (gdeclar.namespace_sym)
                sftemp = nspace_searchmember(&vident[0], gdeclar.namespace_sym);
            else
            {   int sct = SCTglobal;
                if (scope_inNamespace())
                    sct = SCTnspace;
                sftemp = scope_searchinner(&vident[0], sct);
            }
...
static if (0)
{
            ...
            for (Symbol *sx = sftemp; sx; sx = sx.Sfunc.Foversym)
            {
                ...
                if (template_arglst_match(sx.Sfunc.Fptal, s.Sfunc.Fptal))
                    cpperr(EM_explicit_following, &s.Sident[0]);
            }
            goto Lfuncdecl;
        }
    }
    ...
    if (stag && !(member_template & 2))
    {
        ...
        s = symbol_name(&vident[0], SCfunctempl, t);
        type_free(t);
        s.Sflags |= access_specifier;
        s.Sscope = stag;
    ...
    else
    {
        ...
        s = symdecl(&vident[0],t,SCfunctempl,temp_arglist);
        if (!s)
            return false;
        symbol_func(s);
...
Lfuncdecl:
    ...
    funcdef = funcdecl(s,cast(SC)s.Sclass,0,&gdeclar);        // declare function
    f = s.Sfunc;
...
version (DEBUG_XSYMGEN)
{
    ...
    {   symlist_t sl;
        ...
        {
            Symbol *sf = cast(Symbol *)list_ptr(sl);

            // Implement the function sf
            assert(!(sf.Sflags & SFLimplem));
            template_matchfunc(s,sf.Stype.Tparamtypes,4|1,cast(ubyte)TMATCHexact,null,null);
        }
...
{   param_t *pa;
                    ...
                    if (smem)
                    {
                        switch (smem.Sclass)
                        {
                            ...
                            default:
                                if (!flag)
                                    return null;
                                cpperr(EM_typename_expected, t.Tident, &stag.Sident[0]);
                        }
                        t = smem.Stype;
                    }
                    else
    ...
    {
        Symbol *stempl = (cast(typetemp_t *)t).Tsym;
        Classsym *stag;
    ...
    {   type *tc;
        ...
        *tc = *t;
        ...
        {   case TYtemplate:
                ...
                (cast(typetemp_t *)tc).Tsym = (cast(typetemp_t *)t).Tsym;
                goto L1;
...
Symbol *template_createsym(const(char)* id, type *t, Symbol **proot)
{
    ...
    {   // type parameter
        ...
        t.Tident = cast(char *) MEM_PH_STRDUP(id);
        sc = SCtypedef;
    }
    else
...
{   Symbol *root;
    ...
    {   Symbol *s;
        ...
        {   type *t;
            ...
            //printf("\ts = %p\n", s);
            s.Sclass = SCtemplate;
            s.Stemplate = cast(template_t *) MEM_PH_CALLOC(template_t.sizeof);
    ...
    scope_push(root,cast(scope_fp)&findsy,SCTtemparg);
}
...
//printf("D BLsrcpos: %x\n", blklst.BLsrcpos.offsetof);
    ...
    else
    {
        ...
        {   /* Already defined, so create a second 'covered' definition */
            s.Scover = cast(Classsym *)symbol_calloc(vident);
            s = s.Scover;
        }
        else
...
version (PUBLIC_EXT)
    ...
    for (list_t plx = template_xlist; plx; plx = list_next(plx))
    {
        p = cast(char *) list_ptr(plx);
        assert(p);
        insblk2(cast(ubyte*) p,BLarg);
        egchar2();
        eofcnt = 0;
    L3:
        stoken();
...
static if (TX86)
                        s = template_matchfunc(s,pl,true,TMATCHexact,null,null);
else
{
version (PUBLIC_EXT)
...
//
                        if ((se = template_implemented_already( stemp, pl )) !is null)
                            s = se;
                        else
...

  {
    ...
    {   symbol_debug(s);
        ...
        for (sl = s.Stemplate.TMinstances; sl; sl = list_next(sl))
        {   Classsym *si = cast(Classsym *)list_symbol(sl);
            list_t tl;
...
version (PUBLIC_EXT)
{
           ...
           if (template_access == SCglobal && !list_inlist(expand_list, si))
            ...
            auto gclasssave = pstate.STgclass;
...
{
                if (isclassmember(s))
                {   Classsym *stag = cast(Classsym *)s.Sscope;

                    if (stag.Sstruct.Stempsym)
                    {
                        //printf("\tstag '%s' is an instance of template '%s'\n", &stag.Sident[0], &stag.Sstruct.Stempsym.Sident[0]);
...
static if (0)
{
                ...
                {   Symbol *sx;
                    ...
                    else if (template_classname(tmf.name, si))
                    {
                        ...
                        sx = n2_searchmember(si, tmf.name);
                    assert(sx);
                    if (sx.Sclass == SCtemplate)
                    {
                    ...
                    {
                        for (sf = sx; sf; sf = sf.Sfunc.Foversym)
                        {
                            symbol_debug(sf);
                            ...
                            {   Symbol *st;
                                ...
                                for (st = sx; st; st = st.Sfunc.Foversym)
                                {
                                    ...
                                    {   Symbol *sm;
                                        ...
                                        sm = template_matchfunc(st,sf.Stype.Tparamtypes,4|1,TMATCHexact,null,null);
                                        if (tmf.ptal)
                                            pstate.STexplicitSpecialization--;
                                        st.Sfunc.Fbody = save;
                                        if (sm)
...
static if (0)
{
                    ...
                    if (tmf.sclassfriend)
                    {   Classsym *stag = cast(Classsym *)tmf.sclassfriend;

                        Symbol *sx = pstate.STlastfunc;
                        if (sx && tyfunc(sx.Stype.Tty))
                        {
                            //printf("\t'%s' which is a friend of class '%s'\n", &sx.Sident[0], &stag.Sident[0]);
                            list_append(&sx.Sfunc.Fclassfriends, stag);
                            list_append(&stag.Sstruct.Sfriendfuncs, sx);
                        }
                    }
                }
                scope_unwind(nscopes);
...
version (PUBLIC_EXT)
  ...
  if (config.flags4 & CFG4tempinst)
  {
    ...
    {   Symbol *s = list_symbol(sl);
        ...
        s = scope_search(&s.Sident[0],SCTglobal | SCTnspace);
        assert(s);
...
{   Symbol *s;
    ...
    if (stag.Sstruct.Stempsym)        /* if stag was generated from a template */
        ...
        s = stag;
    symbol_debug(s);
    return strcmp(vident,&s.Sident[0]) == 0;
}
        ...
        *pp = p;
        ...
        {   // Looking for an expression of type pt.Ptype
            ...
            {   Symbol *sx = e.EV.Vsym;
                ...
                if (e.PEFflags & PEFaddrmem || e.EV.Voffset)
                    cpperr(EM_bad_template_arg, &sx.Sident[0]);

                if (type_struct(sx.Stype))
                {
                    if (tybasic(sx.Stype.Tty) != tybasic(e.ET.Tnext.Tty) ||
                        sx.Stype.Ttag != e.ET.Tnext.Ttag)
                        cpperr(EM_bad_template_arg, &sx.Sident[0]);
                }
            }
        ...
        {   // Looking for a template-template-parameter
            ...
            {   if (sx.Sclass != SCtemplate)
                {
                    cpperr(EM_class_template_expected, &sx.Sident[0]); // only class templates allowed
                    sx = null;
                }
            }
            p.Psym = sx;
        ...
        {   // Looking for a type-parameter
            ...
            if (tkval == TKgt)
            {
                ...
                {   synerr(EM_num_args,nargs,&s.Sident[0],nargs + 1);      // %d args expected
                    p.Ptype = tserr;
                    tserr.Tcount++;
                }
            }
            ...
            {
                if (!type_specifier(&t))
                    cpperr(EM_type_argument,pt.Pident,&s.Sident[0]);      // must be type-argument
        ...
        if (pt)                         /* if more args coming          */
        {
            ...
            {   int nactual = nargs;
                ...
                nargs += pt.length();
                synerr(EM_num_args,nargs,&s.Sident[0],nactual);    // %d args expected
                break;
            }
        }
        else
...
Classsym *template_inscope(Symbol *s)
{
    ...
    for (; s; s = s.Stemplate.TMpartial)
    {
        ...
        {   // Allow template name as synonym for this instance
            return cast(Classsym *)funcsym_p.Sscope;
        }
    }
    //printf("not in scope\n");
    return null;
...
//if (stempl && stempl.Stype.Tflags & TFforward) printf("\tstempl is forward\n");
       ...
       )
    {
        ...
        const noparsesave = pstate.STnoparse;
        int levelsave = level;
...
//printf("STinexp = %d, STnoparse = %d\n",pstate.STinexp,pstate.STnoparse);
        ...
        {   Classsym *sf = cast(Classsym *)list_ptr(fl);
...
//printf("+instantiating %p, %s\n", stag, stag.Sident);
        token_unget();
        token_setident(cast(char *)&stag.Sident[0]);
...
void template_parse_nobody()
{
    ...
    for ( sl = sl_tmp; sl; sl = list_next(sl) ) {
        si = cast(Classsym *)list_symbol(sl);
...
{   param_t *template_argument_list;
    ...
    if (!(flag & 1))
    {
        ...
        {   /* Generate explicit class definition from source   */
            ...
            token_unget();
            token_setident(&si.Sident[0]);         // set token to be template id
//printf("+instantiating %p '%s')\n", si, si.Sident);
...
private Classsym *template_expand2(Symbol *stempl, param_t *template_argument_list)
{
    ...
    for (sp = stempl; sp; sp = sp.Stemplate.TMpartial)
    {
        ...
        {   // for each instantiation of stempl
            ...
            if (strcmp(tident,&si.Sident[0]) == 0)
            {
                param_free(&template_argument_list);
                if (LOG_EXPAND2) printf("\t\tfound existing instantiation '%s', %p\n", &si.Sident[0], si);
    ...
    if (LOG_EXPAND2) printf("\tcreate new instantiation\n");

    //tident = alloca_strdup(tident);     // original may get overwritten
    {
        size_t len = strlen(tident) + 1;
        char* s = cast(char *)alloca(len);
        tident = cast(char *) memcpy(s,tident,len);
    }
    ...
    uint structflags = stempl.Stemplate.TMflags;
    ...
    if (LOG_EXPAND2) printf("\tnew instantiation on %p is '%s', %p\n", sp, &si.Sident[0], si);
...
{   Symbol *root;
    ...
    enum LOG_CREATESYMTAB = 0;

    if (LOG_CREATESYMTAB) printf("template_createsymtab(%p,%p)\n",ptpl,ptal);

    debug assert(ptpl.length() == ptal.length());
    root = null;
...
static if (LOG_CREATESYMTAB)
{
        ...
        {   // Create an alias for the template-template-argument
            ...
            if (LOG_CREATESYMTAB) printf("\t\tTemplate '%s'\n", &ptal.Psym.Sident[0]);

            for (s = ptal.Psym; s.Sclass == SCalias; s = (cast(Aliassym *)s).Smemalias)
            { }                       // no nested alias's
            (cast(Aliassym *)st).Smemalias = s;
            st.Sclass = SCalias;
...
static if (LOG_CREATESYMTAB)
{
        ...
        if (LOG_CREATESYMTAB) printf("\n");
        st.Stype.Tcount++;
    }
    scope_push(root,cast(scope_fp)&findsy,SCTtempsym);
}
        ...
        match_t matchStage, param_t *ptali, Symbol *stagfriend)
{
    ...
    enum LOG_MATCHFUNC = 0;

static if (LOG_MATCHFUNC)
{
...
static if (0)
{
        ...
        {
            if (LOG_MATCHFUNC) printf("\tmatch with previous function\n");
            break;
        }
    }
    ...
    {   type *tret;
        ...
        stag = cast(Classsym *)stemp.Sscope;
        ...
        {
            sf = cast(Symbol *)list_ptr(castlist);
...
static if (1)
{
            ...
            {
                if (LOG_MATCHFUNC) printf("\tmatch with previous cast function\n");
                break;
            }
}
else
...
printf("sf  : "); type_print(sf.Stype.Tnext);
    ...
    if (sf && !(parsebody & 4))
    {   param_free(&ptal);
        if (LOG_MATCHFUNC) printf("\treturning previous match\n");
        return sf;
    }
...
static if (LOG_MATCHFUNC)
{
    ...
    {   Symbol* ss = cast(Symbol *) list_ptr(l);
        ...
        if (ss.Sclass == SCstruct)
        {
            ...
            {
static if (LOG_MATCHFUNC)
                printf("\tstag '%s' is an instance of template '%s'\n", &ss.Sident[0], &ss.Sstruct.Stempsym.Sident[0]);
                template_createsymtab(
    ...
    if (ses == stemp)
        ...
        scope_push(null, cast(scope_fp)&findsy, SCTtempsym);
    ...
    if (stag && !(parsebody & 4))
    {
        ...
        if (tok.TKval == TKident && template_classname(tok.TKid, stag))
        {
            ...
            if (tk == TKlpar)
            {
                ...
                dt = declar_fix(typ_spec, &vident[0]);
    ...
    else if (sf)
    {
        ...
        else
        {
            ...
            if (sf.Sscope && sf.Sscope.Sclass == SCstruct)
                pstate.STclasssym = cast(Classsym *)sf.Sscope;
        }
    }
...
else
{
    ...
    param_free(&ptal);
    if (LOG_MATCHFUNC)
        dbg_printf("template_matchfunc() match = '%s' %p\n", sf ? &sf.Sident[0] : "null".ptr, sf);
    return sf;

nomatch:
    if (LOG_MATCHFUNC) dbg_printf("template_matchfunc() nomatch\n");
    param_free(&ptal);
    return null;
}
        ...
        type *ptyFormal )
{
    ...
    while (ptyTemplate && ptyFormal) {
        ...
        {
            Symbol *stemplate = (cast(typetemp_t *)ptyTemplate).Tsym;
...
{   int error = 0;
    ...
    {
        if (OTbinary(e.Eoper))
            error |= template_elemdependent(e.EV.E2, ptpl);
        e = e.EV.E1;
    }
    if (e.Eoper == OPvar || e.Eoper == OPrelconst)
    {
        Symbol *s = e.EV.Vsym;
        if (ptpl.search(&s.Sident[0]))
        {
            cpperr(EM_not_simple_id, &s.Sident[0]);
            error = 1;
            e.Eoper = OPconst;
        }
    }
...
//type_print(p.Ptype);
                ...
                {
                    match = cast(ubyte)typematch(t,p.Ptype,0);
...
static if (0)
{
        ...
        else if (tybasic(tpty) == TYtemplate)
        {
            ...
            sprimary = (cast(typetemp_t *)tp).Tsym;
            ...
            if (tybasic(tety) == TYtemplate)
            {
                 ...
                 */
                {   Symbol *se = (cast(typetemp_t *)te).Tsym;
                    if (flags & 1 &&
                        strcmp(&sprimary.Sident[0], &se.Sident[0]) &&
                        !(sprimary.Stype.Tflags & TFforward) &&
                        (se.Stype.Tflags & TFforward))
                        goto nomatch;
//else printf("%s %s %x\n", &sprimary.Sident[0], &se.Sident[0], flags);
else if (flags & 8 && strcmp(&sprimary.Sident[0], &se.Sident[0]))
    goto nomatch;
                }
...
static if (LOG_MATCHTYPE)
{
                    ...
                    Match mx;
                    mx = template_matchtype(pp.Ptype,pe.Ptype,null,ptpl,ptal,flags | 2);
                    if (mx.m < TMATCHexact)
                        goto nomatch;
                    if (!el_match(pp.Pelem,pe.Pelem))
                    {
                        ...
                        if (epar.Eoper == OPvar && OTleaf(earg.Eoper))
                        {
                            char *id = &epar.EV.Vsym.Sident[0];
                            p = ptal.search(id);
                            assert(p);
                            if (p.Pelem)
                            {
...
static if (LOG_MATCHTYPE)
                ...
                p = ptal.search(&sprimary.Sident[0]);
                assert(p);
...
static if (LOG_MATCHTYPE)
{
                    ...
                    if (!el_match(pp.Pelem,pe.Pelem))
                    {
                        ...
                        if (OTleaf(earg.Eoper) && earg.Eoper != OPvar)
                        {
                            if (epar.Eoper == OPvar)
                            {
                                char *id = &epar.EV.Vsym.Sident[0];
                                ...
                                {
                                    elem *ex = el_copytree(earg);
                                    ex = poptelem(typechk(ex, epar.ET));
                                    p.Pelem = ex;
                                }
                            }
                                     ...
                                     !OTleaf(epar.Eoper))
                            {
                                elem *ex = el_copytree(epar);

                                if (tybasic(ex.ET.Tty) == TYident)
                                {   type *t = ex.ET;
                                    ex.ET = template_tyident(t, ptal, ptpl, 0);
                                    type_free(t);
                                }

                                ex = template_resolve_idents(ex, ptal);
                                ex = poptelem(ex);
                                if (!el_match(ex, earg))
                                {   el_free(ex);
                                    goto nomatch;
                    ...
                    {
                        p = ptal.search(&pp.Psym.Sident[0]);
                        if (p)
                        {
...
samety:
            ...
            {
                Match mx;
                mx = template_matchtype(tp.Ttag.Stype,te.Ttag.Stype,null,ptpl,ptal,flags);
                if (mx.m < TMATCHexact)
                    goto nomatch;
            }
            ...
            if (tyfunc(tety))
            {
                ...
                if (ee && ee.Eoper == OPrelconst && ee.ET == testart &&
                    ...
                    testart.Tnext == te &&
                    !(ee.EV.Vsym.Sfunc.Fflags & Finstance)
                   )
                {   Symbol *s = ee.EV.Vsym;
                    Symbol *smatch = null;
                    int ntemplates = 0;
...
static if (LOG_MATCHTYPE)
                        ...
                        {   if (!pe)
                            ...
                            Match mx;
                            mx = template_matchtype(pp.Ptype,pe.Ptype,null,ptpl,ptal2,flags);
                            if (mx.m < TMATCHexact)
                                goto Lnext;
...
static if (LOG_MATCHTYPE)
                ...
                {   if (!pe)
                    ...
                    Match mx;
                    mx = template_matchtype(pp.Ptype,pe.Ptype,null,ptpl,ptal,flags);
                    if (mx.m < TMATCHexact)
                        goto nomatch;
        ...
        else if (!te.Tnext)
        {
            if (tybasic(tpty) == TYmemptr &&
                tybasic(tp.Ttag.Stype.Tty) == TYident)
            {
                p = ptal.search(&tp.Ttag.Sident[0]);
                assert(p);
...
static if (LOG_MATCHTYPE)
    ...
    m.m = cast(ubyte)(match - adjustment);
    return m;
...
private elem *template_resolve_idents(elem *e, param_t *ptal)
{
    if (!OTleaf(e.Eoper))
    {
        e.EV.E1 = template_resolve_idents(e.EV.E1, ptal);
        if (OTbinary(e.Eoper))
            e.EV.E2 = template_resolve_idents(e.EV.E2, ptal);
    }
...
static if (LOG_DEDUCE)
        ...
        else
        {
            ...
            {
static if (0) // C++ 14.8.2.4-17 defaults do not participate in template type parameter deduction
{
                for (; p; p = p.Pnext)
                {   elem_debug(p.Pelem);
                    type_debug(p.Ptype);
...
static if (0)
{
                    ...
                    {
                        char *id = &epar.EV.Vsym.Sident[0];
                        param_t *pa = ptal.search(id);
                        if (pa)
                        {
                            if (!OTleaf(earg.Eoper))
                                goto nomatch;
                            if (flags & 1)      // if partial ordering test
                            {
                                if (pa.Pelem)
                                {
                                    if (earg.Eoper == OPvar &&
                                        pa.Pelem.Eoper == OPvar &&
                                        strcmp(&pa.Pelem.EV.Vsym.Sident[0], &earg.EV.Vsym.Sident[0]) == 0)
                                        goto Lnext;
                                    if (el_match3(earg, pa.Pelem))
                                        goto Lnext;
                                    goto nomatch;
                                }
                                else
                                {
                                    pa.Pelem = el_copytree(earg);
                                }
                            }
                            else
                            {
                                ...
                                if (pa.Pelem && !el_match(pa.Pelem, e))
                                    goto nomatch;
                                pa.Pelem = e;
                            }
                            goto Lnext;
...
Symbol *template_class_match(Symbol *sprimary, param_t *ptali, param_t **pptali)
{
    ...
    enum LOG_CLASS_MATCH = 0;

static if (LOG_CLASS_MATCH)
    printf("template_class_match(sprimary = %p, '%s')\n", sprimary, &sprimary.Sident[0]);
    ...
    {   Classsym *sencl = cast(Classsym *)sprimary.Sscope;
...
static if (LOG_CLASS_MATCH)
        ...
        {
            TMNE *tmne = cast(TMNE *)list_ptr(tl);

            //printf("\t\tTMNE '%s'\n", tmne.name);
            if (strcmp(tmne.name, &sprimary.Sident[0]) != 0)
                continue;
            //printf("\t\tnames match\n");
...
static if (0)
{
        ...
        for (tl = sencltemp.Stemplate.TMexplicit; tl; tl = list_next(tl))
        {
            ...
            TME *tme = cast(TME *)list_ptr(tl);
            Symbol *sx = tme.stempl;

            if (strcmp(&sx.Sident[0], &sprimary.Sident[0]) != 0)
                continue;
            //printf("\t\tnames match\n");
            ...
            if (!sx.Sscope)
            {
                sx.Sflags |= sencl.Sstruct.access;
                sx.Sscope = sencl;
                sx.Stemplate.TMprimary = sprimary;
            }
static if (LOG_CLASS_MATCH)
            printf("\t\tmatch is %p\n", sx);
...
static if (LOG_CLASS_MATCH)
    ...
    if (ambig)
        cpperr(EM_ambiguous_partial, &ambig.Sident[0]);

    if (spartial)
    {
static if (LOG_CLASS_MATCH)
        printf("\tmatch with partial %p '%s' #%d\n", spartial, &spartial.Sident[0], n);
...
void template_explicit_instantiation()
{
    ...
    else
    {
        ...
        {   Classsym *stag = gdeclar.class_sym;
            ...
            sft = n2_searchmember(stag, &vident[0]);
            assert(sft);
            if (sft.Sclass != SCfunctempl)
            {
static if (0) // needs work, as sft.Stype could be a TYmfunc but dt is still a TYnfunc
{
            ...
            sft = nspace_searchmember(&vident[0], gdeclar.namespace_sym);
        else
            sft = scope_search(&vident[0],SCTglobal | SCTnspace);
...
void tme_hydrate(TME **ptme)
{
    ...
    {   TME *tme = cast(TME *)ph_hydrate(cast(void**)ptme);
...
void tmne_hydrate(TMNE **ptmne)
{
    ...
    {   TMNE *tmne = cast(TMNE *)ph_hydrate(cast(void**)ptmne);

        ph_hydrate(cast(void**)&tmne.name);
...
void tmnf_hydrate(TMNF **ptmnf)
{
    ...
    {   TMNF *tmnf = cast(TMNF *)ph_hydrate(cast(void**)ptmnf);

        token_hydrate(&tmnf.tdecl);
        param_hydrate(&tmnf.temp_arglist);
        symbol_hydrate(&tmnf.stempl);
        symbol_hydrate(cast(Symbol **)&tmnf.stag);
    }
}
}
